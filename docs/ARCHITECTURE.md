...
3. **Secrets are process-local.** Browser Cookie values are hidden input and never serialized.
...
The bootstrap trusts `torch.cuda.device_count()` from a verified ROCm build, not the host's physical inventory. It starts one ComfyUI process per container-visible GPU on ports `8188 + index`. Independent jobs are distributed across those ports; a one-GPU container remains fully functional.
